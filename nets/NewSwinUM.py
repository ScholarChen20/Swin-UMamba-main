...
import logging
from .model.vmambav3 import SwinPA
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2],
        ...
        self.attention1 = AttentionModule(in_channels=2 * 48)
        self.attention2 = AttentionModule(in_channels=4 * 48)
        self.attention3 = AttentionModule(in_channels=8 * 48)
        self.attention4 = AttentionModule(in_channels=16 * 48)
    ...
    def forward(self, x):
        ...
        # skip connection and multiscale
        x_ret[1] = self.attention1(x_ret[1])
        x_ret[2] = self.attention2(x_ret[2])
        x_ret[3] = self.attention3(x_ret[3])
        x_ret[4] = self.assp(x_ret[4])
...
class SwinUMambaD(nn.Module):
    ...
    def forward(self, x):
        skips = self.vssm_encoder(x) #([1, 3, 256, 256]), ([1, 96, 64, 64]),([1, 192, 32, 32]),([1, 384, 16, 16]),([1, 768, 8, 8])
        out = self.decoder(skips) #(1,1,256,256) (1,1,64,64)(1,1,32,32) (1,1,16,16)
        return out
...
def load_pretrained_ckpt(
    ...
    # print("ckpt Key:",ckpt['model'].keys())
    ...
    #     #     # kr = kr.replace(f"layers.{i_ds}.downsample", f"downsample.{i_ds}")
...
def load_new_model(use_pretrain: bool = True):
...
def load_SwinPA_UM(use_pretrain: bool = True):
    model = SwinPA().cuda()
    model.apply(InitWeights_He(1e-2))
    model.apply(init_last_bn_before_add_to_0)
    if use_pretrain:
...
if __name__ == '__main__':
    # model1 = load_new_model()  #21.84M 6.206     41.82M  7.45
    model1 = load_SwinPA_UM()  #34.71M 9.48
    input = torch.randn(2, 3, 256, 256).cuda()  # 确保输入在 GPU 上
    ...
    cal_params_flops(model1, size=256, logger=logger)