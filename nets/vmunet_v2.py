from .model.vmambav2 import VSSM
...
class VMUNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size()[1] == 1: # 如果是灰度图，就将1个channel 转为3个channel
            x = x.repeat(1,3,1,1)
        f1, f2, f3, f4 = self.vmunet(x) #  f1 [2, 64, 64, 96]  f4  [2, 8, 8, 768]  [b h w c]
        # b h w c --> b c h w
...
def load_vm2_model():
    pretrain_path = '/home/cwq/MedicalDP/SwinUmamba/swin-umamba/pretrained_ckpt/vmamba_tiny_e292.pth'
...
if __name__ == '__main__':
    pretrained_path = '/home/cwq/MedicalDP/SwinUmamba/swin-umamba/pretrained_ckpt/vmamba_tiny_e292.pth'
    model = VMUNetV2(load_ckpt_path=pretrained_path, deep_supervision=True).cuda()
    model.load_from()
    # x = torch.randn(2, 3, 256, 256).cuda()
    # predict = model(x)
    # cal_params_flops(model,256,logger)
    # print(len(predict),type(predict))
    # print([o.shape for o in predict])  #  deep_supervision true   predict[0] [2, 1, 256, 256] , predict[1] [2, 1, 128, 128] 这两项用于监督
