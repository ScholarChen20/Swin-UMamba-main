...
from .lib.operations import InvertedResidual


from .efficient_scan import EfficientScan, EfficientMerge
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer < 2:
                ...
                self.layers.append(nn.Sequential(
                    Permute(0, 3, 1, 2),
                    *[
                        InvertedResidual(self.dims[i_layer], self.dims[i_layer], expand_ratio=4, se_ratio=0.125, drop_connect_rate=dpr[sum(depths[:i_layer]) + i])
...
class Backbone_EfficientVSSM(EfficientVSSM):
    ...
    def forward(self, x):
        ...
        return outs
    #([1, 1, 256, 256]), torch.Size([1, 1, 128, 128]), torch.Size([1, 1, 64, 64]), torch.Size([1, 1, 32, 32])]