...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True,aux_loss=True,
    ...
    def forward(self, x):
        ...
        if self.aux_loss and self.training:
            x, x2, x3 = self.forward_features_up(x, skip_list)
            x1 = self.forward_final(x)
            return x1, x2, x3   # (1,1,256,256) (1,1,256,256)  (1,1,256,256)
        else:
...
def medmamba_t():
    pretrain = "/home/cwq/MedicalDP/SwinUmamba/swin-umamba/pretrained_ckpt/MedMamba.pth"
    model = MedMamba(load_ckpt_path=pretrain).cuda()
    # model.load_from()
    return model
    # VSSM(depths=[2, 2, 4, 2], dims=[96, 192, 384, 768], num_classes=100).to("cuda")
def medmamba_s():
    VSSM(depths=[2, 2, 8, 2], dims=[96, 192, 384, 768], num_classes=100).to("cuda")
def medmamba_b():
    VSSM(depths=[2, 2, 12, 2], dims=[128, 256, 512, 1024], num_classes=100).to("cuda")
...
if __name__ == '__main__':
    ...
    y = model(x)
    print(y.shape)
    # cal_params_flops(model,256,logger)           #MedMamba 13.3 2.02